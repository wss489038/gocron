...
init() {
    ...
    PACKAGE_DIR='gocron-package'/'arm64-(aarch64)'/${ARCH}-${BINARY_NAME}-package
...
build() {
    ...
    for OS in "${INPUT_OS[@]}";do
        for ARCH in "${INPUT_ARCH[@]}";do
            ...
            env CC=aarch64-linux-gcc CGO_ENABLED=1 GOOS=${OS} GOARCH=${ARCH} go build -ldflags "${LDFLAGS}" -o ${BUILD_DIR}/${BINARY_NAME}-${OS}-${ARCH}/${FILENAME} ${MAIN_FILE}
        done
    done
}
