...
init() {
    ...
    PACKAGE_DIR='gocron-package'/${ARCH}/${ARCH}-${BINARY_NAME}-package
...
build() {
    ...
    for OS in "${INPUT_OS[@]}";do
        for ARCH in "${INPUT_ARCH[@]}";do
            ...
            env CC=musl-gcc CGO_ENABLED=1 GOOS=${OS} GOARCH=${ARCH} go build -ldflags "${LDFLAGS}" -o ${BUILD_DIR}/${BINARY_NAME}-${OS}-${ARCH}/${FILENAME} ${MAIN_FILE}
        done
    done
}
...
package_binary() {
    ...
    for OS in "${INPUT_OS[@]}";do
        ...
        if [[ "${OS}" = "windows" ]];then
            ...
            echo "Zip Command:"
            echo "zip -rq ../${PACKAGE_DIR}/${BINARY_NAME}-${VERSION}-${OS}-${ARCH}.zip ${BINARY_NAME}-${OS}-${ARCH}"
